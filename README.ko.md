...
Taskbar Pixel Pals는 작업 표시줄 위에 작은 픽셀 동물들이 올라오는 Windows 데스크톱 컴패니언 앱입니다.

복잡한 위젯 플랫폼이 아니라, 데스크톱에 가볍고 귀여운 생동감을 더하는 작은 유틸리티를 목표로 만들었습니다.
...
Taskbar Pixel Pals는 작업 표시줄 hover 상태와 preview 상태를 읽고, 그 위치에 맞춰 작은 캐릭터를 띄워서 데스크톱이 조금 더 살아 있는 것처럼 보이게 합니다.
...
| `TaskbarPixelPals.U2OverlaySpike.exe` | 포터블 실행 | 설치 없이 바로 실행하는 단일 실행 파일입니다. |
| `taskbar-pixel-pals-windows-x64.zip` | 수동 압축 해제 | 포터블 빌드를 ZIP으로 묶은 버전입니다. |
...
Taskbar Pixel Pals를 실행하면 설정창이 열리고 오버레이 동작도 함께 시작됩니다.
...
2. `Randomize character per icon`
   작업 표시줄 앱마다 다른 캐릭터를 자동으로 배정하고 싶을 때 켭니다.
3. `Motion`
   기본 모션 스타일을 고르는 영역입니다.
4. `Randomize motion per icon`
   작업 표시줄 앱마다 다른 움직임을 쓰고 싶을 때 켭니다.
5. `Scale`
   오버레이 전체 크기를 `Small`, `Medium`, `Large` 중에서 고릅니다.
6. `Hide overlay while a fullscreen app is active`
   게임, 영상, 전체 화면 작업 중에는 캐릭터가 보이지 않게 하고 싶다면 켭니다.
7. `Normal hover position`
   preview 썸네일이 열리기 전, 일반 작업 표시줄 아이콘 hover 상태에서 캐릭터 위치를 보정합니다.
8. `Preview hover position`
   Windows preview 썸네일이 열린 상태에서, 캐릭터가 그 preview에 걸쳐 앉는 위치를 보정합니다.
9. `Close / Apply`
...
- `Preview hover position`은 썸네일 preview 위에 걸치는 상태용입니다.
- `X`는 좌우 이동입니다.
- `Y`는 상하 이동입니다.
...
또는 앱 아이콘별로 랜덤 캐릭터를 배정할 수도 있습니다.
...
디스플레이 배율, 작업 표시줄 위치, preview 크기, 시스템 환경에 따라 캐릭터 위치가 약간씩 어긋날 수 있습니다.
...
- `Preview hover position`
  preview 썸네일이 열렸을 때, 캐릭터가 그 preview에 걸쳐 보이는 위치를 조정합니다.
...
- `Y`는 상하 이동입니다.
- 현재 UI 기준으로 `Y`가 음수면 아래로, 양수면 위로 움직입니다.
- preview에 걸친 포즈가 너무 높거나 낮아 보이면 `Preview hover position`을 조정하면 됩니다.
...
`Preview hover position X/Y`를 조정해서 preview 썸네일 기준 위치를 미세 보정하면 됩니다.
...
- `Preview hover position`: 썸네일 preview 위에 걸친 상태
